...
export class DeleteReports1601595526348 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
        DROP TRIGGER IF EXISTS increment_daily_posts ON posts;
        DROP TRIGGER IF EXISTS increment_daily_merits ON merits;
        DROP TRIGGER IF EXISTS increment_daily_users ON users;
      `);

    await queryRunner.query(`
        DROP FUNCTION IF EXISTS increment_daily_posts();
        DROP FUNCTION IF EXISTS increment_daily_merits();
        DROP FUNCTION IF EXISTS increment_daily_users();
      `);
