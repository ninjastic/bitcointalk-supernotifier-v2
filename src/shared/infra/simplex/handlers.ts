...
export async function handleChatMessage(
  ...
  if (chatItem.chatInfo.type !== 'direct' || !chatItem.chatInfo.contact.contactId) return;
  ...
  if (text.startsWith('/')) {
    ...
    switch (command[1]) {
      ...
      case '/info': {
        const infoUser = await simpleX.db.getUser(contactId);
        await simpleX.sendMessage(contactId, `\`${JSON.stringify(infoUser)}\``);
        break;
      }
      case '/mentions': {
      ...
      case '/phrase': {
        if (command[2]) {
          ...
          if (isExistent) {
            await simpleX.sendMessage(
              contactId,
              `Error: Phrase *${lowercasePhrase}* already being tracked`,
            );
            break;
          }
      ...
      case '/user': {
        if (command[2]) {
          ...
          if (isExistent) {
            await simpleX.sendMessage(
              contactId,
              `Error: User *${lowercaseUser}* already being tracked`,
            );
            break;
          }
      ...
      case '/list_users': {
        ...
        await simpleX.sendMessage(
          contactId,
          `Tracked users: ${trackedUsers.map(({ username }) => `*${username}*`).join(', ')}`,
        );
        break;
      }
      case '/del_user': {
        if (command[2]) {
          ...
          await simpleX.sendMessage(
            contactId,
            `User *${lowercaseUser}* is no longer being tracked`,
          );
        }
        break;
      }
      ...
      case '/ignoreuser': {
        if (command[2]) {
          ...
          if (isExistent) {
            await simpleX.sendMessage(
              contactId,
              `Error: User *${lowercaseUser}* already being ignored`,
            );
            break;
          }
      ...
      case '/list_ignoreusers': {
        ...
        await simpleX.sendMessage(
          contactId,
          `Ignored users: ${trackedUsers.map(({ username }) => `*${username}*`).join(', ')}`,
        );
        break;
      }
      case '/del_ignoreuser': {
        if (command[2]) {
          ...
          await simpleX.sendMessage(
            contactId,
            `User *${lowercaseUser}* is no longer being ignored`,
          );
        }
        break;
      }
...
export const handlers: Handlers = {
  receivedContactRequest: async (r: CEvt.ReceivedContactRequest, simpleX) => {
    const { contactId } = await simpleX.chat.apiAcceptContactRequest(
  ...
  newChatItems: async (r: CEvt.NewChatItems, simpleX) => {
    for (const chatItem of r.chatItems) {
      if (
        chatItem.chatItem.content.type !== 'rcvMsgContent' ||
        chatItem.chatItem.content.msgContent.type !== 'text'
      )
        continue;
      await handleChatMessage(chatItem, chatItem.chatItem.content.msgContent, simpleX);
    }
  },
