...
metaRouter.get('/', (request: Request, response: Response, next: NextFunction) => {
  void (async () => {
    ...
    const { value: result1Value } = result1.aggregations.posts_notifications_sum as {
      value: number;
    };
    const { value: result2Value } = result2.aggregations.merits_notifications_sum as {
      value: number;
    };
