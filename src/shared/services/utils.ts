...
export function validateTronAddress(addressBase58Check: string) {
  try {
    if (typeof addressBase58Check !== 'string' || addressBase58Check.length !== 34)
      return false;
...
export function shouldNotifyUser(post: Post, user: User, ignoredUsers: IgnoredUser[] = [], ignoredTopics: IgnoredTopic[] = [], ignoredBoards: IgnoredBoard[] = []): boolean {
...
export function isUserMentionedInPost(content: string, user: { username?: string; alternative_usernames?: string[] }, onlyDirectAndQuote?: boolean): {
  isMentioned: boolean;
